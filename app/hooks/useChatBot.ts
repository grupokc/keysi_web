...
export const useChatBot = () => {
  ...
  // Genera una constante para un guid aleatorio
  const generateGuid = () => {
  ...
  const sendMessage = useCallback(
    async (content: string) => {
      ...
      try {
        ...
        if (!from) {
          throw new Error(
            'No se pudo obtener el GUID del agente. Por favor, inicia sesión nuevamente.',
          );
        }

        // Generar conversationId con timestamp de CDMX
        const hoyCDMX = DateTime.now()
  ...
  const addWelcomeMessage = useCallback(() => {
    if (state.messages.length === 0 && state.isConnected) {
      const welcomeMessage: Message = {
        id: '1',
        content:
          '¡Hola! Soy tu asistente virtual de Titan. Estoy conectado y listo para ayudarte con cualquier consulta sobre ventas, productos, bonos o reportes. ¿En qué puedo asistirte hoy?',
        role: 'assistant',
        timestamp: new Date(),
      };
      ...
      const errorMessage: Message = {
        id: '1',
        content:
          'No se pudo establecer la conexión. Por favor, asegúrate de estar autenticado en el sistema.',
        role: 'assistant',
        timestamp: new Date(),
      };
  ...
  const getSuggestionsFromBot = useCallback(
    async (lastAssistantMessage: string) => {
  ...
  const generateLocalSuggestions = useCallback((lastMessage: string) => {
    ...
    // Sugerencias para bonos y comisiones
      ...
      message.includes('comisión') ||
      message.includes('incentivo')
    ) {
      suggestions = [
    ...
    // Sugerencias para prospección
      ...
      message.includes('prospección') ||
      message.includes('prospecto') ||
