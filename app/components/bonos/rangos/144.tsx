...
interface BonosRangos144Props {
  Bono_Periodo: string;
}

const BonosRangos144: React.FC<BonosRangos144Props> = ({ Bono_Periodo }) => {
            ...
            <div className="min-w-full overflow-hidden overflow-x-auto align-middle shadow sm:rounded-lg">
              <h1 className="text-sm font-semibold bg-blue-300 p-4">
                <div>
                  ...
                  {dataBonoRango[0].DIRECTO == 1 ? (
                    <span>Tienes un Agente Productivo.</span>
                  ) : null}
                  {dataBonoRango[0].DIRECTO > 1 ? (
                    <span>Tienes {dataBonoRango[0].DIRECTO} Agentes Productivos. </span>
                  ) : null}
                  {dataBonoRango[0].DIRECTO == 0 ? (
                    <span>No tienes Agentes Productivos.</span>
                  ) : null}
                </div>
              </h1>
                  ...
                  {dataBonoRango.map((bonoRango, index) => {
                    ...
                    if (
                      ...
                      ((index === dataBonoRango.length - 1) ||
                        (index < dataBonoRango.length - 1 && dataBonoRango[index + 1].En_Rango === 0))
                    ) {
                      filaAmarilla = true;
                    }
                    ...
                    if (iCeros < 100 || filaAmarilla) {
                      banda = (
                        <tr className={filaAmarilla ? 'bg-yellow-200' : 'bg-white'} key={index}>
                          ...
                          <td className="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:pl-6">
                            {dollarUSLocale.format(bonoRango.Bono)} %
                          </td>
                          <td className="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:pl-6">
